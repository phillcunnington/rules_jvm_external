...
def _add_outdated_files(repository_ctx, artifacts, repositories):
    ...
    # For time being skipping imported artifacts
    file_string = ""
    for artifact in artifacts:
        if "version" in artifact:
            file_string = file_string + "\n{}:{}:{}".format(artifact["group"], artifact["artifact"], artifact["version"])
...
def _remove_auth_from_file_location(file_location):
    if file_location == None or "%40" not in file_location:
        return file_location
    last_index=file_location.rfind("%40", 0, None)
    first_index=file_location.rfind("/", 0, last_index)
    return file_location[:first_index + 1] + file_location[last_index + 3:]
...
def _remove_auth_from_dep_tree(dep_tree):
    for artifact in dep_tree["dependencies"]:
        updated_artifact = {
            "coursier_file": artifact["file"],
            "file": _remove_auth_from_file_location(artifact["file"])
        }
        if "url" in artifact:
            updated_artifact["url"] = remove_auth_from_url(artifact["url"])
        artifact.update(updated_artifact)
    return dep_tree

def _remove_coursier_file_location_from_dep_tree(dep_tree):
    for artifact in dep_tree["dependencies"]:
        artifact.pop("coursier_file")
    return dep_tree
...
def _coursier_fetch_impl(repository_ctx):
    ...
    dep_tree = _remove_auth_from_dep_tree(dep_tree)
    ...
    dependency_tree_json = "{ \"dependency_tree\": " + repr(_remove_coursier_file_location_from_dep_tree(dep_tree)).replace("None", "null") + "}"
    repository_ctx.template(
        "pin.sh",
